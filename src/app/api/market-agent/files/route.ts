...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const filesApiUrl = `${API_BASE_URL}/agents/${agentName}/files/read/content/lway_market_report.html`;
    ...
    return NextResponse.json({
      ...
      path: `content/lway_market_report.html`,
      agent: agentName
    });
  } catch (error) {
    console.error('Error fetching HTML file:', error);
    return NextResponse.json(
      { error: 'Failed to fetch HTML file from market agent', details: error instanceof Error ? error.message : String(error) },
      { status: 500 }
    );
  }
}
